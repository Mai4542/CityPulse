...
import {
  ...
  FiMapPin, FiChevronLeft,
  FiSearch, FiLogOut, FiImage, FiSettings
} from 'react-icons/fi';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-5 shadow-sm text-center">
          <p className="text-3xl font-black text-slate-900 mb-1">{stats.resolved}</p>
          <p className="text-sm font-bold text-slate-500">تم حلها</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-5 shadow-sm text-center">
          <p className="text-3xl font-black text-slate-900 mb-1">{stats.inProgress}</p>
...
const DashboardContent = ({ setActiveView, reports, loading }) => (
      ...
      <div className="divide-y divide-slate-50">
        {loading ? (
          ...
          reports.slice(0, 5).map((report) => (
            <ReportRow key={report.rawId} report={report} />
          ))
        ) : (
...
const ReportsView = ({ reports, loading }) => {
  const [search, setSearch] = useState('');
  const filtered = reports.filter(
    r => r.title.includes(search) || r.address.includes(search) || r.id.includes(search)
  );
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="divide-y divide-slate-50">
          {loading ? (
            ...
            filtered.map((report) => <ReportRow key={report.rawId} report={report} />)
          ) : (
...
const SettingsView = () => (
  <div className="space-y-6 animate-in fade-in duration-300">
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center text-slate-400 font-bold">
      قريباً...
    </div>
  </div>
);
...
export default function Dashboard() {
  ...
  const stats = {
    ...
    inProgress: reports.filter(r => r.statusType === 'processing' || r.statusType === 'assigned').length,
  };

  const navLinks = [
    { key: 'dashboard', label: 'لوحتي',    icon: FiGrid,     badge: null },
    { key: 'reports',   label: 'بلاغاتي',  icon: FiFileText, badge: reports.length ? String(reports.length) : null },
    { key: 'settings',  label: 'الإعدادات', icon: FiSettings, badge: null },
  ];

  if (activeView === 'new-report') {
    return (
      <div className="h-screen overflow-y-auto bg-[#f8fafc]" dir="rtl" style={{ fontFamily: "'Cairo', sans-serif" }}>
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-[#f8fafc]" dir="rtl" style={{ fontFamily: "'Cairo', sans-serif" }}>
      {mobileOpen && (
        <div
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        {activeView === 'dashboard' && (
          <div className="bg-[#f8fafc] px-4 md:px-8 pt-4 md:pt-8 pb-4 shrink-0">
            <DashboardHeader setActiveView={setActiveView} user={user} stats={stats} />
          </div>
        )}

        <main className="flex-1 overflow-y-auto bg-[#f8fafc]">
          <div className={`w-full ${activeView === 'dashboard' ? 'px-4 md:px-8 pb-8' : 'p-4 md:p-8'}`}>
            {error && (
            ...
            {activeView === 'dashboard' && (
              <DashboardContent setActiveView={setActiveView} reports={reports} loading={loading} />
            )}
            {activeView === 'reports' && <ReportsView reports={reports} loading={loading} />}
